...
(define-function hermite ((point :vec2))
  (hermite point (lambda ((x :vec2)) (virality.shader.hash:fast32/2-per-corner x))))
...
(define-function hermite/derivs ((point :vec2)
  ...
  (mvlet* ((cell (floor point))
           ...
           (grad-x (* grad-x norm))
           (grad-y (* grad-y norm))
           (temp-x (virality.shader.shaping:quintic-hermite
                    (.y vec) (.xy grad-x) (.zw grad-x) (.xy grad-y)
                    (.zw grad-y)))
           (temp-y (virality.shader.shaping:quintic-hermite
                    (.x vec) (.xz grad-y) (.yw grad-y) (.xz grad-x)
                    (.yw grad-x)))
           (noise (virality.shader.shaping:quintic-hermite
                   (.x vec) (.x temp-x) (.y temp-x) (.z temp-x) (.w temp-x)))
           (noise (map-domain noise -0.4419417 0.4419417 0 1))
           (derivs (* (vec2 (virality.shader.shaping:quintic-hermite/derivative
                             (.x vec) (.x temp-x) (.y temp-x) (.z temp-x)
                             (.w temp-x))
                            (virality.shader.shaping:quintic-hermite/derivative
...
(define-function hermite/derivs ((point :vec2))
  (hermite/derivs point (lambda ((x :vec2))
                          (virality.shader.hash:fast32/2-per-corner x))))
...
(define-function hermite ((point :vec3))
  (hermite point (lambda ((x :vec3)) (virality.shader.hash:fast32/3-per-corner x))))
...
(define-function hermite/derivs ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (ival-z igrad-xz igrad-yz (virality.shader.shaping:quintic-hermite
                                      (.z vec) grad-x0 grad-x1 grad-y0 grad-y1
                                      grad-z0 grad-z1))
           (ival-y igrad-xy igrad-zy (virality.shader.shaping:quintic-hermite
                                      (.y vec)
           ...
           (temp-z (virality.shader.shaping:quintic-hermite
                    (.x vec)
           ...
           (derivs (* (vec3 (virality.shader.shaping:quintic-hermite/derivative
                             (.x vec) (.x temp-x) (.y temp-x) (.z temp-x)
                             (.w temp-x))
                            (virality.shader.shaping:quintic-hermite/derivative
                             (.y vec) (.x temp-y) (.y temp-y) (.z temp-y)
