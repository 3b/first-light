...
(define-function polkadot ((point :vec2)
  ...
  (let* ((cell (floor point))
         ...
         (vec (+ (* (.xy hash) (- radius 2))
                 (- (* vec radius) (1- radius)))))
    (* (virality.shader.shaping:falloff-squared-c2 (min (dot vec vec) 1.0)) value)))

(define-function polkadot ((point :vec2)
                           (radius-low :float)
                           (radius-high :float))
  (polkadot point radius-low radius-high (lambda ((x :vec2))
                                           (virality.shader.hash:fast32/cell x))))
...
(define-function polkadot ((point :vec3)
  ...
  (let* ((cell (floor point))
         ...
         (vec (+ (- (* vec radius) (1- radius))
                 (* (.xyz hash) (- radius 2)))))
    (* (virality.shader.shaping:falloff-squared-c2 (min (dot vec vec) 1)) value)))
