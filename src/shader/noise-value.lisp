(in-package #:virality.shader.noise)
...
(define-function value ((point :vec2)
  ...
  (let* ((cell (floor point))
         ...
         (blend (virality.shader.shaping:quintic-curve vec))
         (blend (vec4 blend (- 1 blend))))
    (dot hash (* (.zxzx blend) (.wwyy blend)))))

(define-function value ((point :vec2))
  (value point (lambda ((x :vec2)) (virality.shader.hash:fast32 x))))
...
(define-function value/derivs ((point :vec2)
  ...
  (let* ((cell (floor point))
         (vec (- point cell))
         (hash (funcall hash-fn cell))
         (blend (virality.shader.shaping:quintic-curve/interpolate-derivative vec))
         (out (mix (.xyxz hash) (.zwyw hash) (.yyxx blend))))
    (+ (vec3 (.x out) 0 0)
       (* (- (.yyw out) (.xxz out)) (.xzw blend)))))

(define-function value/derivs ((point :vec2))
  (value/derivs point (lambda ((x :vec2)) (virality.shader.hash:fast32 x))))
...
(define-function value ((point :vec3))
  (value point (lambda ((x :vec3)) (virality.shader.hash:fast32 x))))
  ...
  (mvlet* ((cell (floor point))
    ...
    (+ (vec4 (.x temp1) 0 0 0)
       (* (- (vec4 (.yyw temp1) (.y temp2)) (vec4 (.xxz temp1) (.x temp2)))
          (vec4 (.x blend) (virality.shader.shaping:quintic-curve/derivative vec))))))

(define-function value/derivs ((point :vec3))
  (value/derivs point (lambda ((x :vec3)) (virality.shader.hash:fast32 x))))
...
(define-function value ((point :vec4))
  (value point (lambda ((x :vec4)) (virality.shader.hash:fast32-2 x))))
...
(define-function value-hermite ((point :vec2)
  ...
  (mvlet* ((cell (floor point))
           ...
           (out (virality.shader.shaping:quintic-hermite
                 (.y vec)
...
(define-function value-hermite ((point :vec2)
  ...
  (value-hermite point value-scale gradient-scale normalization-value
                 (lambda ((x :vec2)) (virality.shader.hash:fast32/3-per-corner x))))
...
(define-function value-hermite ((point :vec3)
  ...
  (mvlet* ((cell (floor point))
           ...
           (ival igrad-x igrad-y (virality.shader.shaping:quintic-hermite
...
(define-function value-hermite ((point :vec3)
                                ...
                                (normalization-value :float))
  (value-hermite point value-scale gradient-scale normalization-value
                 (lambda ((x :vec3)) (virality.shader.hash:fast32/4-per-corner x))))
  ...
  (mvlet* ((cell (floor point))
           ...
           (grad-results (mix (1- (* hash 2))
                              (* (inversesqrt (+ (* grad-x grad-x)
                                 ...
                                 1.4142135)
                              blend-value))
           (blend (virality.shader.shaping:quintic-curve (.xy vecs)))
           (blend (vec4 blend (- 1 blend)))
...
(define-function value-perlin ((point :vec2)
  ...
  (value-perlin point blend-value (lambda ((x :vec2))
                                    (virality.shader.hash:fast32/3-per-corner x))))
...
(define-function value-perlin ((point :vec3)
  ...
  (value-perlin point blend-value (lambda ((x :vec3))
                                    (virality.shader.hash:fast32/4-per-corner x))))
