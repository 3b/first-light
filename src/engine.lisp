...
(defun load-initial-scene (core scene-name)
  (let* ((scene-name (or scene-name (option (context core) :initial-scene)))
         (prefab-descriptor (virality.prefab:find-prefab-descriptor scene-name)))
    (virality.prefab:make-prefab-instance core prefab-descriptor)))

(defun initialize-engine (core scene-name)
  (let ((title (option (context core) :title)))
    (log:info :virality "Starting up ~a..." title)
    (setup-live-coding)
    (enable-logging core)
    ...
    (log:info :virality "Finished starting ~a" title)))
...
first, and finally cleaning up."
  (let ((title (option core :title)))
    (log:info :virality "Shutting down ~a..." title)
    (run-epilogue core)
    (virality.gpu:unload-shaders)
    (shutdown-host core)
    (setf (running-p core) nil)
    (makunbound '*core-debug*)
    (log:info :virality "Successfully shut down ~a" title)))
