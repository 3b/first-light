...
(define-function noise/perlin-3d/frag ((uv :vec2)
                                       &uniform
                                       (time :float))
  (let ((noise (vec3 (virality.shader.noise:perlin (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/perlin-surflet-3d/frag ((uv :vec2)
                                               &uniform
                                               (time :float))
  (let ((noise (vec3 (virality.shader.noise:perlin-surflet (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/perlin-improved-3d/frag ((uv :vec2)
                                                &uniform
                                                (time :float))
  (let ((noise (vec3 (virality.shader.noise:perlin-improved (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/perlin-4d/frag ((uv :vec2)
                                       &uniform
                                       (time :float))
  (let ((noise (vec3 (virality.shader.noise:perlin (vec4 (* 10 uv) time (/ time 2))))))
    (vec4 noise 1)))

(define-function noise/cellular-3d/frag ((uv :vec2)
                                         &uniform
                                         (time :float))
  (let ((noise (vec3 (virality.shader.noise:cellular (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/cellular-fast-3d/frag ((uv :vec2)
                                              &uniform
                                              (time :float))
  (let ((noise (vec3 (virality.shader.noise:cellular-fast (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/hermite-3d/frag ((uv :vec2)
                                        &uniform
                                        (time :float))
  (let ((noise (vec3 (virality.shader.noise:hermite (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/simplex-perlin-3d/frag ((uv :vec2)
                                               &uniform
                                               (time :float))
  (let ((noise (vec3 (virality.shader.noise:simplex-perlin (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/simplex-cellular-3d/frag ((uv :vec2)
                                                 &uniform
                                                 (time :float))
  (let ((noise (vec3 (virality.shader.noise:simplex-cellular (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/simplex-polkadot-3d/frag ((uv :vec2)
                                                 &uniform
                                                 (time :float))
  (let ((noise (vec3 (virality.shader.noise:simplex-polkadot
...
(define-function noise/value-3d/frag ((uv :vec2)
                                      &uniform
                                      (time :float))
  (let ((noise (vec3 (virality.shader.noise:value (vec3 (* 10 uv) time)))))
    (vec4 noise 1)))

(define-function noise/value-4d/frag ((uv :vec2)
                                      &uniform
                                      (time :float))
  (let ((noise (vec3 (virality.shader.noise:value (vec4 (* 10 uv) time (/ time 2))))))
    (vec4 noise 1)))
...
(define-function noise/value-hermite-3d/frag ((uv :vec2)
                                              &uniform
                                              (time :float))
  (let ((noise (vec3 (virality.shader.noise:value-hermite
...
(define-function noise/value-perlin-3d/frag ((uv :vec2)
                                             &uniform
                                             (time :float))
  (let ((noise (vec3 (virality.shader.noise:value-perlin (vec3 (* 10 uv) time) 0.5))))
    (vec4 noise 1)))

(define-function noise/polkadot-3d/frag ((uv :vec2)
                                         &uniform
                                         (time :float))
  (let ((noise (vec3 (virality.shader.noise:polkadot (vec3 (* 10 uv) time) 0.0 1.0))))
    (vec4 noise 1)))

(define-function noise/polkadot-box-3d/frag ((uv :vec2)
                                             &uniform
                                             (time :float))
  (let ((noise (vec3 (virality.shader.noise:polkadot-box
...
(define-function noise/cubist-3d/frag ((uv :vec2)
                                       &uniform
                                       (time :float))
  (let ((noise (vec3 (virality.shader.noise:cubist (vec3 (* 10 uv) time) (vec2 0 1.0)))))
    (vec4 noise 1)))

(define-shader noise ()
  (:vertex (virality.shader.texture:unlit/vert-only-uv1 mesh-attrs))
