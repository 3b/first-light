...
(define-function 3d-graph-1 ((i :float)
  ...
  (let* ((offset (vec3 0 0 (* i 0.002)))
         (m3 (mat3 (cos i) 0 (- (sin i))
                   ...
                   (sin i) 0 (cos i)))
         (pos (* m3 offset))
         (h (* 40 (virality.shader.noise:perlin-surflet (+ (* 0.03 pos)
                                                           (vec3 (* 0.5 time))))))
...
(define-function 3d-graph/frag ((uv :vec2)
  ...
  (let ((scale 1))
    (mix (vec4 0)
         color
         (virality.shader.sdf:mask/fill
          (virality.shader.sdf:dist/circle (- (* uv 2 scale) (vec2 scale))
                                           scale)))))
