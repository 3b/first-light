...
;; (v:start-engine :scene 'protect-the-planets:ptp)
...
;; NOTE: Because we don't yet have mesh/sprite rendering order in Virality Engine,
...
(in-package #:virality.shader.user)
...
(u:eval-always
 ...
 (define-shader starfield ()
   (:vertex (virality.shader.texture:unlit/vert virality.shader:mesh-attrs))
...
(v:define-texture white (:texture-2d virality.textures:clamp-all-edges)
...
(v:define-material starfield
    (:profiles (virality.materials:u-mvpt)
     :shader virality.shader.user:starfield
...
(defmethod v:on-component-attach ((self player-movement) actor)
  ...
  (with-accessors ((actor v:actor) (transform transform)) self
    (setf transform (v:component-by-type actor 'transform))))
      ...
      (u:mvlet* ;; TODO: This must be mvlet*
          (;; Deal with deadzones and other bad data around the input vector.
           ...
           (current-translation
            ...
            (v/comp::current (v/comp::translation transform)))
           (vec (clip-movement-vector vec current-translation region-cuboid)))
        (v/comp:translate transform vec))
...
(defmethod v:on-component-attach ((self line-mover) actor)
  (declare (ignore actor))
  (with-accessors ((actor v:actor) (transform transform)) self
    (setf transform (v:component-by-type actor 'v/comp:transform))))
...
(defmethod v:on-component-physics-update ((self line-mover))
  (with-accessors ((context v:context)
    ...
    (when transform
      (v/comp:translate
       transform
       (let* ((local (v/comp:local transform))
...
(defun make-projectile (context translation rotation physics-layer depth-layer
  ...
  (let* ((new-projectile
    ...
    (v/comp:translate projectile-transform translation
                      :instant-p t :replace-p t)
    ;; XXX This interface needs to take a quat here also
    (v/comp:rotate projectile-transform rotation
                   :instant-p t :replace-p t)
    ;; And adjust the scale too.
    (v/comp:scale projectile-transform scale
                  :instant-p t :replace-p t)
    ...
    (setf
     ...
     (v/comp:on-layer (collider projectile)) physics-layer
     ...
     (v/comp::name (sprite projectile)) name
     (v/comp::frames (sprite projectile)) frames
...
(defun make-explosion (context translation rotation scale
  ...
  (let* ((new-explosion
    ...
    (setf
     ...
     (v/comp::name (sprite explosion)) name
     (v/comp::frames (sprite explosion)) frames)

    (v/comp:scale explosion-transform scale
                               :instant-p t :replace-p t)
    (v/comp:translate explosion-transform translation
                                   :instant-p t :replace-p t)
    (v/comp:rotate explosion-transform rotation
                                :instant-p t :replace-p t)
...
(defun possibly-make-explosion-at-actor (actor)
  (let* ((context (v:context actor))
         (actor-transform (v:component-by-type actor
                                                      'v/comp:transform))
         (parent-model (v/comp:model actor-transform))
...
(defmethod v:on-component-attach ((self gun) actor)
  ...
  (with-accessors ((actor v:actor) (emitter-transform emitter-transform)) self
    (setf emitter-transform (v:component-by-type
                             actor 'v/comp:transform))))
    ...
    (cond
      ...
      ((>= cooldown-time (/ fire-period))
       ...
       (u:mvlet ((rx ry (v:get-gamepad-analog
         ...
         (let* ((parent-model (v/comp:model emitter-transform))
                (parent-translation (m4:get-translation parent-model)))
           (unless (or (= rx ry 0.0)
...
(defmethod v:on-component-update ((self asteroid-field))
  (with-accessors ((spawn-period spawn-period)
    ...
    (let ((transform
            (v:component-by-type (v:actor self) 'v/comp:transform)))
...
(v:define-component player-stable ()
  ((%max-lives :accessor max-lives
   ...
   (%mockette-prefab :accessor mockette-prefab
                     ...
                     :initform (virality.prefab::prefab-descriptor
    ...
    (let* ((dir (ecase direction (:left -1) (:right 1)))
           (mockette (first
                      (v:make-prefab-instance
                       ...
                       mockette-prefab
                       :parent stable)))
           (transform (v:component-by-type mockette 'v/comp:transform)))
...
(defmethod v:on-component-attach ((self planet) actor)
  (setf (transform self) (v:component-by-type actor 'v/comp:transform)))
      ...
      ((>= warning-explosion-timer (/ warning-explosion-period))
       ...
       (flet ((zrandom (val)
         ...
         (let* ((er explosion-region)
                ...
                (world-location
                  (v/comp:transform-point transform local-location))
                ...
                (explosion (v:component-by-type (v:actor self)
                                                       'explosion)))
...
(defmethod v:on-component-physics-update ((self time-keeper))
  (with-accessors ((context v:context)
    ...
    (let ((how-far-to-empty (- 1f0 (/ time-left time-max))))
      ...
      (v/comp:scale time-bar-transform
      ...
      (let ((material (v/comp:material time-bar-renderer)))
...
(defmethod process-director-state ((self director)
  ...
  (with-accessors ((context v:context)
    ...
    (let ((lvlmgr (v:component-by-type current-level-ref
                                              'level-manager)))
...
(v:define-prefab "projectile" (:library ptp)
  ...
  (projectile :transform (v:ref :self :component 'v/comp:transform)
              :mover (v:ref :self :component 'line-mover)
              :collider (v:ref :self :component 'v/comp:collider/sphere)
              :sprite (v:ref :self :component 'v/comp:sprite)
              :action (v:ref :self :component 'v/comp:actions))
  (tags :tags '(:dangerous))
  (damage-points :dp 1)
  ...
  (v/comp:collider/sphere :center (v3:zero)
                                       :on-layer :enemy-bullet
                                       :referent (v:ref :self :component 'hit-points)
                                       :radius 15)

  (v/comp:render :material 'sprite-sheet
                              :mode :sprite)
...
(v:define-prefab "player-ship" (:library ptp)
  ...
  ("ship-body"
   ...
   ("exhaust"
    (v/comp:transform :translate (v3:vec 0 -60 0))
    (v/comp:sprite :spec :spritesheet-data
                                :name "exhaust03-01"
                                :frames 8)
    (v/comp:render :material 'sprite-sheet
...
(v:define-prefab "generic-explosion" (:library ptp)
  (explosion :sprite (v:ref :self :component 'v/comp:sprite))
  (v/comp:sprite :spec :spritesheet-data
...
(v:define-prefab "warning-wave-sign" (:library ptp)
  "Not used yet."
  (v/comp:transform :translate (v3:vec 0 0 (dl :sign))
                     :scale 512f0)
  ("sign"
   (v/comp:static-mesh :location '((:core :mesh) "plane.glb"))
   (v/comp:render :material 'warning-wave)))
...
(v:define-prefab "warning-mothership-sign" (:library ptp)
  ...
  ("sign"
   (v/comp:static-mesh :location '((:core :mesh) "plane.glb"))
   (v/comp:render :material 'warning-mothership)))
...
(v:define-prefab "game-over-sign" (:library ptp)
  ...
  ("sign"
   (v/comp:static-mesh :location '((:core :mesh) "plane.glb"))
   (v/comp:render :material 'game-over)))
  ...
  ("bug-todo:implicit-transform:see-trello"
   ...
   ("time-bar-root"
    ...
    ("time-display"
     ...
     (v/comp:render :material `(time-bar ,(gensym "TIME-BAR-MATERIAL-")))))))
...
(v:define-prefab "level-0" (:library ptp)
  ...
  (("planet-0" :link ("/generic-planet" :from ptp))
   (v/comp:transform :translate (v3:vec 0 100 (dl :planet))
                      :scale 0.9f0)
   (v/comp:sprite :spec :spritesheet-data
                   :name "planet01"))
  (("planet-1" :link ("/generic-planet" :from ptp))
   (v/comp:transform :translate (v3:vec -200 -100 (dl :planet))
                      :scale 0.9f0)
   (v/comp:sprite :spec :spritesheet-data
                   :name "planet02"))
...
sequencing."
  ...
  (("camera" :copy ("/cameras/ortho" :from ptp-base))
   (v/comp:transform :translate (v3:vec 0 0 (dl :camera))))
